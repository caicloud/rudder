...
func (s *ingressLister) List(selector labels.Selector) (ret []*networkingv1beta1.Ingress, err error) {
	...
	list, err := s.client.NetworkingV1beta1().Ingresses(v1.NamespaceAll).List(listopt)
	if err != nil {
		return nil, err
	}
...
func (s ingressNamespaceLister) List(selector labels.Selector) (ret []*networkingv1beta1.Ingress, err error) {
	...
	list, err := s.client.NetworkingV1beta1().Ingresses(s.namespace).List(listopt)
	if err != nil {
		return nil, err
	}
	for i := range list.Items {
...
func (s ingressNamespaceLister) Get(name string) (*networkingv1beta1.Ingress, error) {
	return s.client.NetworkingV1beta1().Ingresses(s.namespace).Get(name, v1.GetOptions{})
}
